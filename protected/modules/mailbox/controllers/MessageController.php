...
class MessageController extends RController {
    ...
    public function actionGetusers() {
        $users = Yii::app()->db->createCommand()->select("users.username")
                ->from('users')
                ->join("authassignment", "authassignment.userid=users.id")
                ->where("authassignment.itemname=:itemname", array(':itemname' => $_POST['user_type']))
                ->queryAll();
