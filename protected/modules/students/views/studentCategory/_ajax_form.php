...
<div id="student-categories_form_con" class="client-val-form">
    ...
    <?php elseif (!$model->isNewRecord): ?>    <h3 id="update_header">Update Student Category <?php
        echo
        $model->id;
        ?>  </h3>
    <?php endif; ?>
    <?php
    $val_error_msg = 'Error.StudentCategories was not saved.';
    ...
    <div id="ajax-form"  class='form' style="margin-left:3px;">
        <?php
        $formId = 'student-categories-form';
        $actionUrl = ($model->isNewRecord) ? CController::createUrl('studentCategory/ajax_create') : CController::createUrl('studentCategory/ajax_update');
    ...
    ', NULL, array('class' => 'errorsum notification errorshow png_bg')); ?>    <p class="note">Fields with <span class="required">*</span> are required.</p>
        ...
        <div class="row buttons" style="width:30%; clear:both;"><br /><br />
            <?php
            echo CHtml::submitButton($model->isNewRecord ? 'Submit' : 'Save', array('class' =>
                'formbut'));
            ?>    </div>
